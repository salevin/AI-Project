...
// "join" followed by the two chain numbers.
	...
	{
		// Use "No Duplicates Breadth First Search"
		// Runs in 199 expanded nodes
		Vector succ = new Vector();
		ChainState cs = (ChainState)state;

		// Iterate through all of the chains
        for (int i=0; i<cs.chains.size(); i++){
            Chain currentChain = (Chain)cs.chains.elementAt(i);
			// Open all loop chains
            if (currentChain.loop) {
				succ.addElement(new StateActionPair("open loop", open(cs, i, 0)));
			}
            else {
				// Check if there are open links
                if (cs.openLinks>0) {
					// If there are, close the chain straight or as a loop
					succ.addElement(new StateActionPair("close straight", close(cs, i, false)));
					succ.addElement(new StateActionPair("close loop", close(cs, i, true)));
				}
                else {
					// If there aren't open links, create more
					succ.addElement(new StateActionPair("open", open(cs,i,0)));
				}
				// For all other chains, allow the current chain to be joined to them.
				if (cs.chains.size() > 1 && cs.openLinks>0 && !currentChain.loop){
					for (int j =0; j<cs.chains.size(); j++) {
						Chain otherChain = (Chain)cs.chains.elementAt(j);
						if (currentChain!=otherChain && !otherChain.loop) {
							succ.addElement(new StateActionPair("join", join(cs, i, j)));
						}
					}
				}
            }
	...
	{
    /* Hint: Consider the relaxed problem in which you have "piles" of links without worrying
     * about whether a link is open or closed or a chain is straight or looped.
     * So you have n piles each of a specified size. In the relaxed problem a move is
     * to take any number of links from one pile and move to another. It should be easy
